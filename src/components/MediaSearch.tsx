...
const REFINEMENT_OPTIONS = {
  ...
  show: [
    ...
    { label: "More action", prompt: "with more action" },
    { label: "More dramatic", prompt: "but more dramatic" },
    { label: "More comedy", prompt: "but funnier" },
    { label: "More romance", prompt: "with more romance" },
  ]
};
...
export function MediaSearch() {
  ...
  const handleSearch = async (searchPrompt: string, isRefinement = false) => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke(
        activeTab === 'movie' ? 'search-movies' : 'search-shows', 
  ...
  const handleTabChange = (value: string) => {
    setActiveTab(value as 'movie' | 'show');
    setResults([]);
    setLastPrompt("");
  };
